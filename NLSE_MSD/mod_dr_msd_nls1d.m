%% modified drive file used for troubleshooting
clearvars; close all; format long;clc;
...
u0init = [ sqrt(mu) * tanh( sqrt(mu) * x' ) + offset*ones(npts,1) ; zeros(npts,1) ];
...
for k=1:nmax
    ...
    p = bicgstab(Jx,-fx,bictol,bicsteps); %***
    fprintf ('%d     %e     \n',k-1,norm(fx) );
